...
public class PlaceOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (validUser != null && validUser.isValidUser()) {
            ...
            String destinationCountry = request.getParameter("country");
            String destinationState = request.getParameter("state");

            Order order = (Order) request.getSession().getAttribute("order");
            order.setDestinationCountry(destinationCountry);
            order.setDestinationCity(destinationState);
            order.setCustomerId(Integer.parseInt(customerId));

            ArrayList products = (ArrayList) request.getSession().getAttribute("products");

            boolean success = OrderServices.saveOrder(order, products);
            if (success) {
